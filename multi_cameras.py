...
classifier_model_path = 'models/region_object_classifier_result3_94.pth' # третая модель для предсказания нарушений в украшениях
...
def run_jewelry_detector(frame, camera_id = None):
    ...
    if camera_id is not None:
        print(f"Камера - {camera_id}:Результаты обнаружения украшений:")
    else:
        print(f"Результаты обнаружения украшений:")
...
def main():
    ...
    while True:
        ...
        # For testing if the rtsp server cameras work
        # frame_name = f"images/test_records/camera_{args.camera_id}_{datetime.now()}.png" 
        # cv2.imwrite(frame_name, cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2BGR))
        # break

        # Используем первую модель для обнаружения людей
        results = model_for_cut(frame_rgb, classes=[0], verbose=False)
        
        results_jewelry = run_jewelry_detector(frame_rgb, args.camera_id)
        if results_jewelry is not None:
            ...
            print("No violance in decoration was detected!")
